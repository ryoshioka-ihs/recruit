...
;(function ($) {
  ...
  eachGetInputName = function(selectors) {
    for (var i = 0; i < selectors.length; i++) {
      names.push(selectors[i]);
    }
  };

  $.fn["mpAjaxZip"] = function(settings, zip_type, _btn_id) {
    ...
    var myZip2addr = {
      'double': {
        ...
        4: function() {
          AjaxZip2.zip2addr(names[0], names[1], names[2], names[3]);
        },

        5: function() {
          AjaxZip2.zip2addr(names[0], names[1], names[2], names[3] , "" , names[4]);
        }
      },
