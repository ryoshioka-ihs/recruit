...
function hideFinishedDateSeminarStop() {
	...
	$sessions.each( function() {
		...
		var status = ('.session .status');//statusの位置を取得


		if (today.getTime() > session_apply.getTime()) {
			$sessions_status.closest('td').text('受付終了');//これを有効にすると、全部のstatusが受付終了にされてしまう
			//jQuery('.session .status').text('受付終了');//これを有効にすると、終了した締切日に受付状況「受付終了」が表示される
			}
